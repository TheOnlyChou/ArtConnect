...
BEGIN
    ...
    -- 3. Vérifier que le membre n'a aucune reservation existante
    -- pour les ateliers demandés (quel que soit le statut, car la PK
    -- interdit plusieurs lignes pour le meme membre et le meme atelier)
    SELECT COUNT(*)
    INTO v_already_1
    FROM Booking
    WHERE id_workshop = p_id_workshop_1
    ...
    IF v_already_1 > 0 OR v_already_2 > 0 OR v_already_3 > 0 THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Erreur : une reservation existe deja pour au moins un des ateliers';
    END IF;
    ...
    -- 5. Insertion atomique de trois nouvelles reservations
    INSERT INTO Booking (
        id_workshop,
        id_communityMember,
...
-- Test call example (manual execution only):
-- CALL sp_register_member_to_multiple_workshops(5, 1, 2, 3, '2026-06-20 10:00:00');